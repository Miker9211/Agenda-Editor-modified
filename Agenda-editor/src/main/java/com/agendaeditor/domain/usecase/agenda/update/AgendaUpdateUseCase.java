package com.agendaeditor.domain.usecase.agenda.update;
...
@AllArgsConstructor
public class AgendaUpdateUseCase implements UseCase<AgendaCommand, AgendaResponse> {

    private final AgendaService agendaService;
    @Override
    public AgendaResponse execute(AgendaCommand command) throws JsonProcessingException {
        return agendaService.updateAgenda(command.getId(), buildDto(command))
                .map(agendaDTO -> {
                    if(agendaDTO != null){
